...
<%@ taglib prefix = "spring" uri = "http://java.sun.com/jsp/jstl/core" %>
  ...
  <script>
	function changeLoginStatus(userId, loginStatus){
			$.ajax({
				url : 'changeUserLoginStatus',
    ...
    <div class="additional">
      <div class="user-card">
        ...
        <div class="points center">
	        <spring:if test="${user.userRole eq '2'}">
	          User Role: GENERAL
	        </spring:if>
	         <spring:if test=" ${user.userRole eq '1'}">
	          User Role: ADMIN
	        </spring:if>
        </div>
      ...
      <div class="more-info">
		...
		<spring:url var = "urlDeleteUser" value ="/admin/deleteUser">
			<spring:param name="contactId" value = "${user.userId}"></spring:param>
		</spring:url>
		
		 <div class="coords" style="margin-bottom: 5px;">
          User Login Status:
         	 <select onchange = "changeLoginStatus(${user.userId}, $(this).val())">
         	 <spring:if test = "${user.userLoginStatus eq '1'}">
          	    <option value = "1" selected>Active</option>
	     		<option value = "2">Blocked</option>
	     	</spring:if>
	     	 <spring:if test = "${user.userLoginStatus eq '2'}">
